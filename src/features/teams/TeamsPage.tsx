...
function RoleEditor({teamId,member,canEdit,onSaved}:{teamId:string;member:TeamMember;canEdit:boolean;onSaved:()=>void}) {
  ...
  async function save(event:FormEvent<HTMLFormElement>){
    ...
    try{await updateTeamMemberRole(teamId,member.participant_id,value);setEditing(false);onSaved()}
...
function TeamWorkspace({ cohort, manage, editing, setEditing, deleting, setDeleting }: {
  ...
  useEffect(() => {
    ...
    void loadTeams(cohort.id).then(async data => {
      ...
      if (active) setResult({ teams: [], roster: [], candidates: [], my_participant_id:null, counts:{}, revision, error: cause instanceof Error ? cause.message : '팀 정보를 불러오지 못했습니다.' })
    })
    return () => { active = false }
