...
function TeamCard({team,members,manage,counts,editing,deleting,onEdit,onDelete}:{team:Team;members:TeamMember[];manage:boolean;counts:TeamDocumentCounts|null|undefined;editing:boolean;deleting:boolean;onEdit:()=>void;onDelete:()=>void}){
  ...
  return <article className={manage?'team-overview-card':'team-overview-card student-team-card'}><div className="team-overview-heading"><span className="badge">{teamStages[team.stage]}</span><span className="field-help">{members.length}명</span></div>
    ...
    {!manage&&<section className="student-team-roster" aria-label="팀원 역할"><h3>팀원 역할 · {members.length}명</h3>{members.length?<ul>{members.map(member=><li key={member.participant_id}><strong>{member.full_name}</strong><span className={member.is_leader?'badge status-active':'badge'}>{member.is_leader?'팀장':'팀원'}</span><span>담당 분야(희망 직무) · {jobGroups[member.job_group]}</span>{!member.is_active&&<span className="badge">비활성</span>}</li>)}</ul>:<p className="field-help">배정된 팀원이 없습니다.</p>}</section>}
    <details className="team-overview-details"><summary>{manage?'팀원·직무·링크 자세히':'직무 구성·외부 링크 자세히'}</summary><p className="field-help">{jobSummary(members)}</p>
      {manage&&(members.length?<ul className="team-roster">{members.map(member=><li key={member.participant_id}><strong>{member.full_name}</strong> {member.is_leader&&<span className="badge status-active">팀장</span>} {!member.is_active&&<span className="badge">비활성</span>}<span>{member.department} · {jobGroups[member.job_group]}</span></li>)}</ul>:<p className="field-help">배정된 팀원이 없습니다.</p>)}
...
export function TeamsPage() {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">TEAM WORKSPACE</p><h1>{manage ? '팀 관리' : '워크스페이스'}</h1><p className="muted">{manage ? '프로그램별 팀을 구성하고 팀장과 프로젝트 정보를 관리합니다.' : '소속 팀과 팀원 역할, 프로젝트 자료를 한곳에서 확인하세요.'}</p></div></div>
