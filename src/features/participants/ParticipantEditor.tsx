...
export function ParticipantEditor({ cohortId, participant, onSaved, onCancel }: {
  ...
  return <section className="panel editor-panel" aria-labelledby="participant-editor-title">
    ...
    <form className="cohort-form" onSubmit={submit}><fieldset disabled={busy}>
      ...
      <label htmlFor="participant-email">이메일 *</label><input id="participant-email" type="email" value={values.email} onChange={event => change('email', event.target.value)} maxLength={254} required />
      ...
      {!participant?.profile_id && <p className="field-help">지금은 참가 정보만 등록됩니다. 로그인 계정 생성·연결은 다음 단계에서 지원합니다.</p>}
