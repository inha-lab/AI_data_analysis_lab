...
function ParticipantWorkspace({ cohort, onEditingChange }: { cohort: Cohort; onEditingChange: (editing: boolean) => void }) {
  ...
  return <>
    <div className="participant-summary"><p><strong>{cohort.name}</strong><span className="muted">{loading || error ? ' · 현황 확인 중' : ` · 참여 중 ${activeCount}명 · 비활성 ${participants.length - activeCount}명`}</span></p><Button disabled={Boolean(editor)} onClick={() => { updateEditor('new'); setNotice('') }}><Plus size={16} aria-hidden="true" /> 참가자 등록</Button></div>
    ...
    <section className="panel participants-panel" aria-label="참가자 목록"><div className="list-toolbar">
      ...
      <Button className="button-secondary" onClick={reload} disabled={loading} aria-label="참가자 목록 새로고침"><RefreshCw size={16} aria-hidden="true" /></Button></div>
