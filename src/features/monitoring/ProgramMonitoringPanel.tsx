...
export function ProgramMonitoringPanel({cohortId}:{cohortId:string}){
  ...
  return <section className="panel monitoring-panel"><div className="section-heading"><h2>프로그램 진행 현황</h2><Button className="button-secondary" onClick={()=>setRevision(value=>value+1)}>새로고침</Button></div>
    {loading?<p role="status">팀 제출 현황을 불러오고 있습니다.</p>:result.error?<p className="form-error" role="alert">{result.error}</p>:data&&<>
      <div className="stats-grid"><article className="stat-card"><p>활성 참가자</p><strong>{data.active_participants}</strong><span>명</span></article><article className="stat-card"><p>팀</p><strong>{data.team_count}</strong><span>개 팀</span></article><article className="stat-card"><p>기획서 제출</p><strong>{data.proposal_submitted} / {data.team_count}</strong><span>{proposalRate(data.proposal_submitted,data.team_count)}</span></article><article className="stat-card"><p>기획서 검토 대기</p><strong>{data.proposals_awaiting_review}</strong><span>건</span></article><article className="stat-card"><p>산출물 등록 팀</p><strong>{data.deliverable_submitted_teams} / {data.team_count}</strong><span>총 {data.deliverable_count}건</span></article><article className="stat-card"><p>보고서 검토 대기</p><strong>{data.reports_awaiting_review}</strong><span>건</span></article><article className="stat-card"><p>검토할 이슈·지원 요청</p><strong>{data.teams.reduce((sum,team)=>sum+team.reports_needing_attention,0)}</strong><span>제출 보고서</span></article></div>
      <h3>팀별 현황</h3>{!data.teams.length?<p className="empty-state">이 프로그램에 등록된 팀이 없습니다.</p>:<div className="monitoring-table-wrap"><table className="monitoring-table"><thead><tr><th>팀·주제</th><th>단계</th><th>기획서</th><th>일일 제출</th><th>주간 제출</th><th>산출물</th><th>작성 중</th><th>검토 대기</th><th>이슈·지원 요청</th></tr></thead><tbody>{data.teams.map(team=><tr key={team.id}><td><Link className="text-link" to={`/teams/${team.id}/full-report`}>{team.name} →</Link><small>{team.topic||'주제 미등록'}</small></td><td>{teamStages[team.stage]}</td><td><Link className="text-link" to={`/teams/${team.id}/proposal`}>{team.proposal_status==='reviewed'?'검토 완료':team.proposal_status==='submitted'?'제출':team.proposal_status==='draft'?'작성 중':'미작성'} →</Link></td><td><Link className="text-link" to={`/teams/${team.id}/reports`}>{team.daily_submitted}건</Link></td><td><Link className="text-link" to={`/teams/${team.id}/reports`}>{team.weekly_submitted}건</Link></td><td><Link className="text-link" to={`/teams/${team.id}/deliverables`}>{team.deliverable_count}건</Link></td><td>{team.report_drafts}건</td><td><Link className="text-link" to={`/teams/${team.id}/reports?status=submitted`}>{team.reports_awaiting_review}건</Link></td><td><Link className="text-link" to={`/teams/${team.id}/reports`}>{team.reports_needing_attention}건</Link></td></tr>)}</tbody></table></div>}
      <div className="section-heading"><h3>검토 대기 기획서 · {data.proposal_review_queue.length}건</h3>{data.proposal_review_queue.length>10&&<Button className="button-secondary" onClick={()=>setShowAllProposals(value=>!value)}>{showAllProposals?'접기':'전체 보기'}</Button>}</div>
      {!data.proposal_review_queue.length?<p className="field-help">검토를 기다리는 기획서가 없습니다.</p>:<ul className="monitoring-review-queue">{(showAllProposals?data.proposal_review_queue:data.proposal_review_queue.slice(0,10)).map(item=><li key={item.team_id}><Link to={`/teams/${item.team_id}/proposal`} className="text-link">{item.title} →</Link><span>{item.team_name} · 제출 {formatScheduleTime(item.submitted_at)} (KST)</span></li>)}</ul>}
      <div className="section-heading"><h3>검토 대기 보고서 · {data.review_queue.length}건</h3>{data.review_queue.length>10&&<Button className="button-secondary" onClick={()=>setShowAllQueue(value=>!value)}>{showAllQueue?'접기':'전체 보기'}</Button>}</div>
      {!data.review_queue.length?<p className="field-help">검토를 기다리는 보고서가 없습니다.</p>:<ul className="monitoring-review-queue">{(showAllQueue?data.review_queue:data.review_queue.slice(0,10)).map(item=><li key={item.id}><Link to={`/teams/${item.team_id}/reports?status=submitted&report=${item.id}`} className="text-link">{item.title} →</Link><span>{item.team_name} · {reportTypeLabels[item.report_type]} {item.round_number}회차 · 제출 {formatScheduleTime(item.submitted_at)} (KST)</span></li>)}</ul>}
