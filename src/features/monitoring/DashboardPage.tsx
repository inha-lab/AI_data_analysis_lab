...
function ProfessorDashboard() {
  ...
  const counts = [
    { label: '전체 기수', count: cohorts.length },
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">PROGRAM OVERVIEW</p><h1>프로그램 대시보드</h1><p className="muted">기수별 운영 현황을 확인하고 다음 프로그램을 준비하세요.</p></div><Link to="/cohorts?new=1" className="button">새 기수 만들기 <ArrowUpRight size={18} aria-hidden="true" /></Link></div>
    {error ? <div className="notice" role="alert"><p>{error}</p><Button onClick={reload}>다시 시도</Button></div> : <>
      <div className="stats-grid">{counts.map(item => <article className="stat-card" key={item.label}><p>{item.label}</p><strong>{loading ? '—' : item.count}</strong><span>기수</span></article>)}</div>
      <section className="panel"><div className="section-heading"><h2><CalendarDays size={20} aria-hidden="true" /> 최근 등록한 기수</h2><Link to="/cohorts" className="text-link">전체 보기 →</Link></div>
