...
interface ProgramDetails { team: Team | null; members: TeamMember[]; deliverables: Deliverable[]; proposalStatus:Proposal['status']|null; reportCounts:{draft:number;submitted:number;reviewed:number}|null; error: string; deliverablesError: string; proposalError:string; reportsError:string }
    ...
    void Promise.all([
      ...
      const entries=await Promise.all(rows.map(async (row):Promise<[string,ProgramDetails]>=>{
        try {
          ...
          if(!team)return [row.id,{team:null,members:[],deliverables:[],proposalStatus:null,reportCounts:null,error:'',deliverablesError:'',proposalError:'',reportsError:''}]
          const members=workspace.roster.filter(member=>member.team_id===team.id)
          const [proposal,reports,deliverables]=await Promise.allSettled([loadProposal(team.id),loadReports(team.id),loadDeliverables(team.id)])
          const reportItems=reports.status==='fulfilled'?reports.value.reports:null
          return [row.id,{team,members,proposalStatus:proposal.status==='fulfilled'?proposal.value.proposal?.status??null:null,reportCounts:reportItems?{draft:reportItems.filter(item=>item.status==='draft').length,submitted:reportItems.filter(item=>item.status==='submitted').length,reviewed:reportItems.filter(item=>item.status==='reviewed').length}:null,deliverables:deliverables.status==='fulfilled'?deliverables.value.items:[],error:'',proposalError:proposal.status==='rejected'?'기획서 현황을 불러오지 못했습니다.':'',reportsError:reports.status==='rejected'?'보고서 현황을 불러오지 못했습니다.':'',deliverablesError:deliverables.status==='rejected'?'산출물 현황을 불러오지 못했습니다.':''}]
        } catch(cause) { return [row.id,{team:null,members:[],deliverables:[],proposalStatus:null,reportCounts:null,error:cause instanceof Error?cause.message:'팀 정보를 불러오지 못했습니다.',deliverablesError:'',proposalError:'',reportsError:''}] }
      }))
