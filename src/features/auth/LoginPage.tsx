...
export function LoginPage() {
  ...
  if (!loading && profile) {
    ...
    const canReturn = (profile.role === 'professor' && (requested === '/cohorts' || requested === '/participants'))
      || ((profile.role === 'professor' || profile.role === 'student') && (requested === '/schedules' || requested === '/teams' || /^\/teams\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/proposal$/i.test(requested ?? '')))
