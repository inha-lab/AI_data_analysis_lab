...
import { deliverableCategories } from '@/features/deliverables/deliverable-model'
...
function ReportWorkspace({teamId}:{teamId:string}){
  ...
  const {team,proposal,reports,members,deliverables}=data
  ...
  return <>
    ...
    <article className="full-report"><header><h1>{fullReportTitle(data)}</h1><p>생성 일시: {formatKst(generatedAt)} KST</p></header>
      ...
      <section><h2>팀원 · {members.length}명</h2>{members.length?<ul>{members.map((member,index)=><li key={`${member.full_name}-${index}`}>{member.full_name}{member.is_leader?' (팀장)':''} · {member.department} · 역할: {member.role_title||'미지정'}{!member.is_active?' · 비활성':''}</li>)}</ul>:<p>팀원이 없습니다.</p>}</section>
      ...
      <section><h2>산출물 · {deliverables.length}건</h2>{deliverables.length?deliverables.map(item=><article className="full-report-entry" key={item.id}><span className="badge">{deliverableCategories[item.category]}</span><h3>{item.title}</h3><p className="proposal-text">{item.description||'설명 없음'}</p><p className="field-help">최근 제출 {item.submitted_name} · {formatKst(item.submitted_at)} KST</p>{safeTeamUrl(item.url)&&<a className="text-link" href={safeTeamUrl(item.url)!} target="_blank" rel="noopener noreferrer">자료 링크 ↗</a>}</article>):<p>등록된 산출물이 없습니다.</p>}</section>
    </article>
