...
export function CohortEditor({ cohort, onSaved, onCancel }: { cohort?: Cohort; onSaved: (value: Cohort) => void; onCancel: () => void }) {
  ...
  return <section className="panel editor-panel" aria-labelledby="editor-title">
    <h2 id="editor-title">{cohort ? '기수 정보 수정' : '새 기수 만들기'}</h2>
    <p className="muted">기수별 운영 기간과 상태를 설정해 주세요.</p>
    <form onSubmit={submit} className="cohort-form"><fieldset disabled={busy}>
      <label htmlFor="cohort-name">기수명 <span aria-hidden="true">*</span></label>
      ...
      <label htmlFor="cohort-description">기수 소개</label><textarea id="cohort-description" rows={4} maxLength={2000} value={description} onChange={event => setDescription(event.target.value)} placeholder="운영 목적이나 안내사항을 입력하세요." />
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="button-row"><Button type="submit">{busy ? '저장 중…' : cohort ? '변경 저장' : '기수 생성'}</Button><Button className="button-secondary" onClick={onCancel}>취소</Button></div>
    </fieldset></form>
  </section>
}
