...
export function CohortsPage() {
  ...
  return <>
    ...
    <div className={editor ? 'cohort-workspace with-editor' : 'cohort-workspace'}><section className="panel" aria-label="기수 목록">
      <div className="list-toolbar"><div><label className="sr-only" htmlFor="cohort-search">기수명 검색</label><input id="cohort-search" type="search" placeholder="기수명 검색" value={search} onChange={event => setSearch(event.target.value)} /></div>
      ...
      {loading ? <p className="empty-state" role="status">기수 목록을 불러오고 있습니다.</p> : error ? <div className="empty-state" role="alert"><p>{error}</p><Button onClick={reload}>다시 시도</Button></div>
        : !visible.length ? <div className="empty-state"><h2>{cohorts.length ? '검색 결과가 없습니다.' : '첫 번째 기수를 만들어 주세요.'}</h2><p>{cohorts.length ? '검색어나 상태 필터를 바꿔 보세요.' : '기수명과 운영 기간을 등록하면 프로그램 관리를 시작할 수 있습니다.'}</p></div>
