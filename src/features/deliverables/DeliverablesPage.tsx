...
function Workspace({teamId}:{teamId:string}){
  ...
  return <>
    ...
    <section className="panel"><div className="section-heading"><h2>등록된 산출물 · {items.length}건</h2>{!manage&&<Button disabled={busy} onClick={()=>choose('new')}>산출물 등록</Button>}</div>
      ...
      {!shown.length?<p className="empty-state">{items.length?'검색 결과가 없습니다.':'등록된 산출물이 없습니다.'}</p>:<div className="deliverable-list">{shown.map(item=><div key={item.id}><article className="deliverable-row"><div><span className="badge">{deliverableCategories[item.category]}</span><h3>{item.title}</h3><p className="proposal-text">{item.description||'설명 없음'}</p><p className="field-help">최근 제출 {item.submitted_name} · {formatScheduleTime(item.submitted_at)} (KST)</p></div><div className="button-row">{safeTeamUrl(item.url)&&<a href={safeTeamUrl(item.url)!} className="text-link" target="_blank" rel="noopener noreferrer">링크 열기 ↗</a>}<Button className="button-secondary" aria-expanded={commentTarget===item.id} onClick={()=>setCommentTarget(current=>current===item.id?null:item.id)}>코멘트 {commentTarget===item.id?'닫기':'보기'}</Button>{!manage&&<><Button className="button-secondary" disabled={busy} onClick={()=>choose(item)}>수정</Button><Button className="button-secondary" disabled={busy||dirty} onClick={()=>void remove(item)}>삭제</Button></>}</div></article>{commentTarget===item.id&&<CommentsPanel key={item.id} teamId={team.id} deliverableId={item.id}/>}</div>)}</div>}
    </section>
