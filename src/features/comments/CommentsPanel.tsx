...
type Comment = { id:string;team_id:string;proposal_team_id:string|null;report_id:string|null;deliverable_id:string|null;author_id:string;author_name:string;body:string;created_at:string;updated_at:string }
...
export function CommentsPanel({teamId,reportId,deliverableId}:{teamId:string;reportId?:string;deliverableId?:string}) {
  ...
  async function save(){
    ...
    const {error:failure}=deliverableId
      ?await supabase.rpc('AD_save_deliverable_comment',{p_team:teamId,p_deliverable:deliverableId,p_comment:editing?.id??null,p_version:editing?.updated_at??null,p_body:value})
      :await supabase.rpc('AD_save_comment',{p_team:teamId,p_proposal:!reportId,p_report:reportId??null,p_comment:editing?.id??null,p_version:editing?.updated_at??null,p_body:value})
    setBusy(false)
  ...
  return <section className="panel comments-panel"><div className="section-heading"><h2>코멘트 · {comments.length}건</h2><Button className="button-secondary" disabled={busy} onClick={reload}>새로고침</Button></div>
    ...
    {manage&&<div className="cohort-form"><label htmlFor={`comment-body-${deliverableId??reportId??'proposal'}`}>{editing?'코멘트 수정':'코멘트 작성'}</label><textarea id={`comment-body-${deliverableId??reportId??'proposal'}`} rows={4} maxLength={4000} disabled={busy} value={body} onChange={event=>setBody(event.target.value)} /><p className="field-help">{body.length.toLocaleString()} / 4,000자{!deliverableId?' · 검토 완료 처리와 별도로 저장됩니다.':''}</p><div className="button-row"><Button disabled={busy} onClick={()=>void save()}>{editing?'수정 저장':'코멘트 등록'}</Button>{editing&&<Button className="button-secondary" disabled={busy} onClick={cancel}>취소</Button>}</div></div>}
