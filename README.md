...
React·TypeScript·Vite, Tailwind, shadcn/ui용 별칭 설정, 공개 소개 화면, Supabase 연결, GitHub CI를 포함합니다. 이메일·비밀번호 인증, 세션 복원·로그아웃, 앱 권한 조회, 보호 라우트, 교수용 대시보드와 기수 조회·생성·수정을 구현했습니다. 참가자·팀·프로젝트 관리, 평가, PWA 설치·오프라인 기능, 자동 배포는 후속 구현 대상입니다.
...
- 공개 키 연결 검증과 `AD_profiles`, `AD_cohorts` 테이블·RLS 생성을 완료했습니다. 지정된 기존 Auth 계정에 교수 역할을 연결했습니다. 다른 업무 테이블은 후속 구현 대상입니다.
