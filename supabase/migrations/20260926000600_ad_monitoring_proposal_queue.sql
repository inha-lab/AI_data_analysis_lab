...
begin
  ...
  with team_rows as (
    select t.id,t.name,t.topic,t.stage,
      ...
      (select count(*) from public."AD_reports" r where r.team_id=t.id and r.status='submitted') as reports_awaiting_review,
  ...
  select jsonb_build_object(
    ...
    'proposals_awaiting_review',(select count(*) from team_rows where proposal_status='submitted'),
    'proposal_review_queue',coalesce((select jsonb_agg(jsonb_build_object('team_id',t.id,'team_name',t.name,'title',p.title,'submitted_at',p.submitted_at) order by p.submitted_at asc,t.id) from public."AD_proposals" p join public."AD_teams" t on t.id=p.team_id where t.cohort_id=p_cohort and p.status='submitted'),'[]'::jsonb),
    ...
    'reports_awaiting_review',(select coalesce(sum(reports_awaiting_review),0) from team_rows),
    'review_queue',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'team_id',t.id,'team_name',t.name,'report_type',r.report_type,'round_number',r.round_number,'title',r.title,'submitted_at',r.submitted_at) order by r.submitted_at asc,r.id) from public."AD_reports" r join public."AD_teams" t on t.id=r.team_id where t.cohort_id=p_cohort and r.status='submitted'),'[]'::jsonb),
    'teams',coalesce((select jsonb_agg(jsonb_build_object('id',id,'name',name,'topic',topic,'stage',stage,'proposal_status',proposal_status,'proposal_title',proposal_title,'daily_submitted',daily_submitted,'weekly_submitted',weekly_submitted,'report_drafts',report_drafts,'reports_awaiting_review',reports_awaiting_review,'reports_needing_attention',reports_needing_attention,'deliverable_count',deliverable_count) order by name) from team_rows),'[]'::jsonb),
