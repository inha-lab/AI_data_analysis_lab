begin;
create function public."AD_program_monitoring"(p_cohort uuid)
...
begin
  ...
  select jsonb_build_object(
    ...
    'teams',coalesce((select jsonb_agg(jsonb_build_object('id',id,'name',name,'topic',topic,'stage',stage,'proposal_status',proposal_status,'proposal_title',proposal_title,'daily_submitted',daily_submitted,'weekly_submitted',weekly_submitted,'report_drafts',report_drafts,'reports_needing_attention',reports_needing_attention) order by name) from team_rows),'[]'::jsonb),
...
$$;
revoke all on function public."AD_program_monitoring"(uuid) from public,anon,authenticated;
grant execute on function public."AD_program_monitoring"(uuid) to authenticated;
commit;
